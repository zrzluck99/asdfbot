...
import os, aiohttp, random, heapq, json
from typing import Optional, Dict, List, Tuple
from PIL import Image, ImageDraw, ImageFont, ImageFilter
from src.libraries.maimai.maimaidx_music import total_list, compute_ra
from src.libraries.maimai.maimai_network import mai_api
from src.libraries.image_range import get_qq_logo
from src.libraries.maimai.static_lists_and_dicts import platename_to_file, pnconvert
...
class DrawBest(object):
    ...
    def __init__(self, sdBest:BestList, dxBest:BestList, nickname:str, plate:str, userid:str):
        self.sdBest = sdBest
        self.dxBest = dxBest
    ...
    async def _drawBestList(self, img:Image.Image, sdBest:BestList, dxBest:BestList):
        itemW = 164
        itemH = 88
        ...
        for num in range(0, len(sdBest)):
            ...
            temp = await mai_api.open_cover(chartInfo.idNum)
    ...
    async def draw(self):
        try:
            with open('src/users/' + self.userid + '.json', "r") as f:
        ...
        try:
            PlateImg = Image.open(user_plate_dir).convert('RGBA')
        except:
        ...
        await self._drawBestList(self.img, self.sdBest, self.dxBest)
    ...
    async def getDir(self):
        await self.draw()
        return self.img

async def generate(payload: Dict, userid: str) -> Tuple[Optional[Image.Image], int]:
    async with aiohttp.request("POST", "https://www.diving-fish.com/api/maimaidxprober/query/player", json=payload) as resp:
        ...
        sd_best.sort()
        dx_best.sort()
        pic = await DrawBest(sd_best, dx_best, obj["nickname"], obj["plate"], userid).getDir()
        return pic, 0

async def generateap40(player_data,userid) -> Image.Image:
    sd_best = BestList(25)
    dx_best = BestList(15)
    ...
    sd_best.sort()
    dx_best.sort()
    pic = await DrawBest(sd_best, dx_best, player_data["nickname"], player_data["plate"],userid).getDir()
    return pic

async def generateb40_by_player_data(player_data,userid) -> Image.Image:
    sd_best = BestList(25)
    dx_best = BestList(15)
    for rec in player_data['records']:
        if total_list.by_id(rec["song_id"]).cn_version == "舞萌DX2024":
