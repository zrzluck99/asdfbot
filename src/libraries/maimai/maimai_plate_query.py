...
class DrawPlate(object):
    def __init__(self):
        ...
        self.level_img_list = {
            f"{level}": Image.open(f"{self.assets_path}{level}.png")
            for level in ["10", "10+", "11", "11+", "12", "12+", "13", "13+", "14", "14+", "15"]
        }
        self.base_img_list = [
    ...
    def update_settings(self, user_qq_id: str, plate: Plate):
        self.user_qq_id = user_qq_id
        self.plate = plate

    async def draw_one_music(self, music_chart: MusicChart, record: BestRecord, mtype: str) -> Image.Image:
        ...
        f = self.finish_img.convert("RGBA") if Plate.single_achieved(mtype, record) else self.unfinish_img.convert("RGBA")
        cover.alpha_composite(f)
            
        base.paste(cover,(5,5),cover)
        
        overlay_img = self.ui_img_list.get(mtype, None)
        if overlay_img:
            base.paste(cover,(int((base.size[0]-cover.size[0])/2),int((base.size[1]-cover.size[1])/2)),cover)

        return base

    async def draw_rank_list(self, plateobj: Plate) -> Image.Image:
        records = plateobj.get_lists_by_level()

        # 计算行数，每行10个
        lines = sum((len(records[key]) - 1) // 10 + 1 for key in records if records[key])
        img = Image.new('RGBA', (1952, lines*160), color = (0, 0, 0, 0))
        
        line, row = 0, 0
        
        for level, mlist in records:
            head = self.level_img_list[level].convert("RGBA")
            img.paste(head, (30, line*160+15), head)
            for music_chart, record in mlist:
                if row == 10:
                    row = 0
                    line += 1
                song_img = await self.draw_one_music(record)
